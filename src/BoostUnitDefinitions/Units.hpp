...
#define ADD_UNIT_IO(NAME, NSTR, SSTR)       \
  ...
  {                                         \
  inline std::string                        \
  name_string(const boost::units::t::NAME&)   \
  {                                         \
    return NSTR;                            \
  }                                         \
  inline std::string                        \
  symbol_string(const boost::units::t::NAME&) \
  {                                         \
    return SSTR;                            \
  }                                         \
  }                                         \
...
ADD_UNIT(percent, make_scaled_unit<dimensionless, scale<10, static_rational<-2>>>::type);
...
// Inverse Length
ADD_UNIT(inverse_kilometer, divide_typeof_helper<dimensionless, kilometer>::type);
ADD_UNIT(inverse_meter, divide_typeof_helper<dimensionless, meter>::type);
ADD_UNIT(inverse_centimeter, divide_typeof_helper<dimensionless, centimeter>::type);
ADD_UNIT(inverse_millimeter, divide_typeof_helper<dimensionless, millimeter>::type);
ADD_UNIT(inverse_micrometer, divide_typeof_helper<dimensionless, micrometer>::type);
ADD_UNIT(inverse_nanometer, divide_typeof_helper<dimensionless, nanometer>::type);
ADD_UNIT(inverse_picometer, divide_typeof_helper<dimensionless, picometer>::type);
ADD_UNIT(inverse_femtometer, divide_typeof_helper<dimensionless, femtometer>::type);
...
ADD_UNIT(inverse_kilometer_squared, divide_typeof_helper<dimensionless, kilometer_squared>::type);
ADD_UNIT(inverse_meter_squared, divide_typeof_helper<dimensionless, meter_squared>::type);
ADD_UNIT(inverse_centimeter_squared, divide_typeof_helper<dimensionless, centimeter_squared>::type);
ADD_UNIT(inverse_millimeter_squared, divide_typeof_helper<dimensionless, millimeter_squared>::type);
...
ADD_UNIT_IO(joule_per_centimeter_squared, "joule per centimeter squared",
            "J cm^-2");
ADD_UNIT_IO(joule_per_meter_squared, "joule per meter squared",
            "J m^-2");
...
ADD_UNIT_IO(watt_per_centimeter_squared, "watt per centimeter squared",
            "W cm^-2");
ADD_UNIT_IO(watt_per_meter_squared, "watt per meter squared",
            "W m^-2");
...
ADD_UNIT(
    nanowatt_per_centimeter_squared_per_steradian,
...
typedef t::joule_per_centimeter_squared_per_steradian::dimension_type IntegratedRadiance;
...
d2a(quantity<U, T> a)
{
  return quantity<t::rad, T>(i::rad * quantity_cast<T>(quantity<t::dimless, T>(a)));
}
...
// 4. convert to return type
quantity<t::dimless, T>
a2d(quantity<U, T> a)
{
  return quantity<t::dimless, T>(i::dimless * quantity_cast<T>(quantity<t::radian, T>(a)));
}
...
// for example, boost::units::t::radian is a scaled base unit based on angle::radian_base_unit,
// so it is not implicitly convertible to/from si::plane_angle, even through
// they represent the same unit. This is true for all of the base units as well.
// boost::units::t::second is based on si::second_base_unit, and cannot be implicitly converted
// to/from si::time.
// below are a list of conversions between types that represent the same unit that the user
// could reasonably expect to be the "same"
#define ADD_IMPLICITLY_CONVERTIBLE_UNITS(A, B) \
namespace boost { \
namespace units { \
  template<> struct is_implicitly_convertible<A,B> : boost::true_type{};\
  template<> struct is_implicitly_convertible<B,A> : boost::true_type{};\
} \
}

// base units
ADD_IMPLICITLY_CONVERTIBLE_UNITS( boost::units::t::radian, angle::radian_base_unit );
ADD_IMPLICITLY_CONVERTIBLE_UNITS( boost::units::t::steradian, angle::steradian_base_unit);
...
ADD_IMPLICITLY_CONVERTIBLE_UNITS( boost::units::t::centimeter_squared, cgs::area );
...
template <typename U, typename T>
boost::units::quantity<typename boost::units::divide_typeof_helper<boost::units::t::dimensionless, U>::type, T>
